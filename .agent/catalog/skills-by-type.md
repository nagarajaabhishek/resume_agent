# Skills by Type

This catalog index allows AI agents to O(1) discover the correct skill paths for the Resume_Agent repository.

## Generation Skills
Skills focused on content creation, tailoring, and LaTeX compilation logic.

| Skill Name | Description | Path |
|---|---|---|
| **Resume Generation** | A modular skill for generating high-impact, ATS-optimized resumes using the XYZ strategy and LaTeX templates. | [`.agent/skills/resume_generation/SKILL.md`](../skills/resume_generation/SKILL.md) |
